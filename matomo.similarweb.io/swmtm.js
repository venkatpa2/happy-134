...
if (typeof window.Matomo !== "object") {
    window.Matomo = (function() {
                    ...
                    for (ax = 0; ax < L.length; ax++) {
                        if (x(aA)) {
                            ...
                            if (av[aA]) {
                                ...
                                var aC = "The method '" + aA + '\' was not found in "_matomoPaq" variable.  Please have a look at the Matomo tracker documentation: https://developer.matomo.org/api-reference/tracking-javascript';
                                ao(aC);
                                if (!aB) {
                    ...
                    if (ay[aw] && ay[aw][0]) {
                        ...
                        if (at === au) {
                            ...
                            if (az[au] > 1 && au !== "addTracker" && au !== "enableLinkTracking") {
                                ao("The method " + au + ' is registered more than once in "_matomoPaq" variable. Only the last call has an effect. Please have a look at the multiple Matomo trackers documentation: https://developer.matomo.org/guides/tracking-javascript-guide#multiple-piwik-trackers')
                            }
                            az[au]++
                        }
                    }
        ...
        function ah(av, au) {
            var at = new T(av, au);
            L.push(at);
            _matomoPaq = c(_matomoPaq, E);
            for (H = 0; H < _matomoPaq.length; H++) {
                if (_matomoPaq[H]) {
                    aj(_matomoPaq[H])
                }
            }
...
(function() {
    function b() {
        if ("object" !== typeof _matomoPaq) {
            return false
        }
        var c = typeof _matomoPaq.length;
        if ("undefined" === c) {
            return false
        }
        return !!_matomoPaq.length
    }
